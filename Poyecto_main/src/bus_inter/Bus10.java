...
public class Bus10 extends Thread {
    ...
    int retraso;
    int[][] corde = {
        {320, 123}, {290, 119}, {260, 115}, {220, 113}, {190, 110}, {160, 110}, {130, 110}, {100, 110}, {100, 120},
        ...
        {920, 535}, {980, 520}, {950, 500}, {930, 480}, {910, 470}, {900, 430}, {920, 400},
        {930, 360}, {940, 330}, {950, 310}, {960, 280}, {910, 255}, {880, 245}, {860, 240},
        {810, 215}, {790, 205}, {750, 190}, {730, 180}, {700, 170}, {670, 165}, {640, 160},
        {600, 155}, {580, 150}, {550, 145}, {520, 140}, {490, 130}, {460, 128}, {420, 128}, {390, 128}, {360, 125}};
    ...
    public Bus10(String nombre, JLabel label, int retraso) {
        this.nombre = nombre;
        this.retraso = retraso;
        this.label = label;
    }

    public void imprimirCoordenadasParadas() {
        System.out.println("Coordenadas de las paradas del Bus 10:");
        for (int i = 0; i < coordeParada.length; i++) {
            System.out.println("Parada " + (i + 1) + ": (" + coordeParada[i][0] + ", " + coordeParada[i][1] + ")");
        }
    }

    public int[][] getCoordeParada() {
        return coordeParada;
    }
    ...
    public void run() {
        ...
        while (true) {
            for (int i = 0; i < corde.length; i++) {
                System.out.println("Bus 10 avanza");
                ...
                for (int j = 0; j < coordeParada.length; j++) {
                    if (corde[i][0] == coordeParada[j][0] && corde[i][1] == coordeParada[j][1]) {
                        try {
                            System.out.println("Bus 10 hace una parada en la " + coordeParada);
                            Thread.sleep(5000);
                        } catch (InterruptedException ex) {
                            Logger.getLogger(Bus10.class.getName()).log(Level.SEVERE, null, ex);
                        }
