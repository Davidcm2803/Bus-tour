...
public class Bus2 extends Thread {
    ...
    int retraso;
    int[][] corde = {
        {930, 360}, {940, 330}, {950, 310}, {960, 280},
    ...
    public Bus2(String nombre, JLabel label, int retraso) {
        this.nombre = nombre;
    ...
    public void run() {
        ...
        while (true) {
            for (int i = 0; i < corde.length; i++) {
                System.out.println("Bus 2 avanza");
                ...
                for (int j = 0; j < coordeParada.length; j++) {
                    if (corde[i][0] == coordeParada[j][0] && corde[i][1] == coordeParada[j][1]) {
                        try {
                            System.out.println("Bus 2 hace una parada en la " + coordeParada);
                            Thread.sleep(5000);
                        } catch (InterruptedException ex) {
                            Logger.getLogger(Bus2.class.getName()).log(Level.SEVERE, null, ex);
                        }
