...
public class Bus5 extends Thread {
    ...
    int[][] corde = {
        ...
        {960, 280},{910, 255},{880, 245}, {860, 240}, {810, 215}};
    ...
    public Bus5(String nombre, JLabel label, int retraso) {
        this.nombre = nombre;
            ...
            for (int i = 0; i < corde.length; i++) {
                ...
                for (int j = 0; j < coordeParada.length; j++) {
                    if (corde[i][0] == coordeParada[j][0] && corde[i][1] == coordeParada[j][1]) {
                        try {
                            System.out.println("Bus 5 hace una parada en la " + coordeParada);
                            Thread.sleep(5000);
                        } catch (InterruptedException ex) {
                            Logger.getLogger(Bus10.class.getName()).log(Level.SEVERE, null, ex);
                        }
