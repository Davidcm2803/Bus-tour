...
public class Bus8 extends Thread {
    ...
    int retraso;
    int[][] corde = {
        {520, 140}, {490, 130}, {460, 128}, {420, 128}, {390, 128}, {360, 125}, {320, 123}, {290, 119},
    ...
    public Bus8(String nombre, JLabel label, int retraso) {
        this.nombre = nombre;
            ...
            for (int i = 0; i < corde.length; i++) {
                ...
                for (int j = 0; j < coordeParada.length; j++) {
                    if (corde[i][0] == coordeParada[j][0] && corde[i][1] == coordeParada[j][1]) {
                        try {
                            System.out.println("Bus 8 hace una parada en la " + coordeParada);
                            Thread.sleep(5000);
                        } catch (InterruptedException ex) {
                            Logger.getLogger(Bus10.class.getName()).log(Level.SEVERE, null, ex);
                        }
